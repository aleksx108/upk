<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between gap-4">
            ...
            <a href="{{ route('personnel.index') }}" class="text-sm text-gray-700 hover:text-gray-900">
                {{ __('Back to list') }}
            </a>
        </div>
    </x-slot>
    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
            <div class="p-4 sm:p-8 bg-white shadow sm:rounded-lg">
                <form method="POST" action="{{ route('personnel.store') }}" class="space-y-6" novalidate enctype="multipart/form-data">
                    @csrf

                    @include('personnel.partials.form', ['personnel' => null])
